...
class ExpertNotificationsScreen extends StatefulWidget {
  ...
  const ExpertNotificationsScreen({super.key, required this.user});

  @override
  State<ExpertNotificationsScreen> createState() => _ExpertNotificationsScreenState();
}

class _ExpertNotificationsScreenState extends State<ExpertNotificationsScreen> {
  bool _loading = true;
  ...
  Future<void> _fetchNotifications() async {
    ...
    final list = await ApiService.getExpertNotifications(expertId: widget.user.id);
    if (mounted) {
      setState(() {
        _notifications = list;
        _loading = false;
  ...
  Future<void> _markAllRead() async {
    final ok = await ApiService.markAllExpertNotificationsRead(expertId: widget.user.id);
    if (ok) {
      _fetchNotifications();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('All expert alerts marked as read.'), backgroundColor: AppTheme.success),
        );
      }
    }
  }
  ...
  Future<void> _deleteNotification(String id) async {
    ...
    if (ok) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Alert dismissed.'), backgroundColor: AppTheme.success),
        );
      }
    }
  }
  ...
  String? _extractAssignmentId(String text) {
    final exp = RegExp(r'(ORD-[A-Za-z0-9-]+|AAH-[A-Za-z0-9-]+|ACE-[A-Za-z0-9-]+)');
  ...
  void _navigateToOrder(String assignmentId) async {
    ...
    if (mounted) {
      if (assignments.isNotEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Task $assignmentId not found or already completed.')),
        );
      }
    }
  }
  ...
  IconData _getIconForNotification(String title, String message) {
    ...
    if (t.contains('allocat') || t.contains('assigned') || t.contains('new task')) {
      return Icons.assignment_add;
    }
    if (t.contains('revision') || t.contains('re-work') || t.contains('qa')) {
      return Icons.published_with_changes_rounded;
    }
    if (t.contains('turnitin') || t.contains('solution') || t.contains('deliver')) {
      return Icons.verified_rounded;
    }
    if (t.contains('deadline') || t.contains('urgent') || t.contains('sla')) {
  ...
  Color _getColorForNotification(String title, String message) {
    final t = '${title.toLowerCase()} ${message.toLowerCase()}';
    if (t.contains('allocat') || t.contains('assigned')) {
      return AppTheme.primary;
    }
    if (t.contains('revision') || t.contains('urgent')) {
      return AppTheme.danger;
    }
    if (t.contains('turnitin') || t.contains('solution') || t.contains('complete')) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Expert Alerts (${_notifications.length})'),
        actions: [
          if (unreadCount > 0)
            IconButton(
              ...
              children: [
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Search task allocations and alerts...',
                    ...
                    ? Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 12),
                            Text(
                              _filter == 'Unread' ? 'No unread expert alerts' : 'No notifications found',
                            ...
                            Text(
                              _searchCtrl.text.isNotEmpty
                                  ...
                                  : 'New task allocations and review notices will appear here.',
                            ...
                            return Dismissible(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  borderRadius: BorderRadius.circular(14),
                                  border: Border.all(
                                    color: n.isRead ? AppTheme.border : color.withValues(alpha: 0.4),
                                  ),
                                ),
                                child: Column(
